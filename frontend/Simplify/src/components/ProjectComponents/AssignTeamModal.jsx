...
const AssignTeamModal = ({ teamId, setTeamId }) => {
  ...
  return (
    <div>
      <div
        onClick={handleOpen}
        className="text-white py-2 bg-secondaryBlue text-lg text-center hover:bg-blue hover:cursor-pointer hover:ease-in-out duration-500"
      >
        <h6>Assign a team to project if ready</h6>
      </div>
      <p className="mt-4 text-center">Or do so later</p>
      <Modal
              ...
              <>
                <div>
                  ...
                  {teams.map((team) => (
                    <div
                      onClick={() => {
                        setTeamId(team._id), handleClose();
                      }}
                      key={team._id}
                      className={`py-2 px-4 shadow-md mb-2 ${
                        teamId === team._id ? "border-r-4 border-blue" : ""
